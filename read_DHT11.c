...
int 	RETRY_WAIT = 2000;			// wait in msec between retries
uint8_t	MCU_START_LOW_WAIT = 20;		// MCU start low level signal length in msec
uint8_t	BIT_MARKER = 42;			// Decision (the bit is 0 or 1) level in usec.
uint8_t	MAX_USEC_BIT_WAIT = 150;		// MAX wait for reading signal changes in usec
